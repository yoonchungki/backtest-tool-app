...
서버에서 자동으로 해줌 (/api/cron_update_all). PRESETS(가격 데이터)와 시가총액만 Vercel Blob에
갱신함 - "미국ETF비교"용 데이터(api/usetf.py)는 애초에 localStorage에만 저장하는 기기별 보조
캐시라(index.html의 PEN_US_ETF_OVERRIDE_KEY 주석 참고) 서버에서 갱신해도 각 기기에 반영될 방법이
없어서 자동화 대상에서 뺐음 - 그건 여전히 브라우저에서 버튼을 눌러야 갱신됨.
